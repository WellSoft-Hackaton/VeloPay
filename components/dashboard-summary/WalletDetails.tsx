...
export function WalletDetails({ onClose, open }: { onClose: () => void; open: boolean }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="flex h-[580px] max-h-[85vh] flex-col rounded-3xl bg-card sm:max-w-md">
        ...
        <div className="flex w-full flex-1 flex-col items-center pt-4">
          ...
          <h2 className="mb-6 text-center text-2xl font-bold text-foreground">Wallet Details</h2>
          <Details
            values={[
              {
                label: "Address",
